...
class w2v():
    ...
    def validation(self,X,Y,kind):
        ...
        print('validating...')
        fold_n=2
        folder = StratifiedKFold(n_splits=fold_n,random_state=0)
        ...
        for j,(train_idx, test_idx) in enumerate(list(folder.split(X,Y))):
            ...
            X_test = X[test_idx]
            y_test = Y[test_idx]
        # folds = list(StratifiedKFold(Y,random_state=0,n_splits=fold_n))
        # score=np.zeros(fold_n)
    ...
    def train_w2v(self, filename):
        ...
        model = word2vec.Word2Vec(sentences, size=self.size, window=100)  # 训练模型; 注意参数window 对结果有影响 一般5-100
