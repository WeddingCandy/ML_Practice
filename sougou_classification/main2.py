...
if __name__ == '__main__':
    ...
    if order == 'train w2v model': #训练WV的model
        totalname =root_path+ 'writefile.csv' #纯文本文件路径
        classob.train_w2v(totalname)
        exit()
    elif order == 'getvec': #利用生成的model得到文档的WV的向量，使用求和平均法
        trainname = root_path + 'train_querylist.csv'
        testname = root_path + 'test_querylist.csv'
    ...
    genderdata = np.loadtxt(open(label_genderfile_path, 'r',encoding='gb18030')).astype(int)
    agedata = np.loadtxt(open(label_agefile_path, 'r',encoding='gb18030')).astype(int)
    educationdata = np.loadtxt(open(label_edufile_path, 'r',encoding='gb18030')).astype(int)
