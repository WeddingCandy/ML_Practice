...
class RNNTextClassifier():
    ...
    def add_wordembedding_layer(self):
        embedding = tf.get_variable("encoder",
                                    [self.vocab_size,self.embedding_size], # embedding的样式
                                    dtype=tf.float32,
                                    initializer=tf.random_uniform_initializer(-1.0,1.0)) #初始值，采用
                                                                                # random_uniform_initializer 初始随机值
        embedded = tf.nn.embedding_lookup(embedding, self._pointer) #转成X的样式
    ...
    def lstm_cell(self):
        ...
        # 获取LSTMCell，num_units 每格
        return tf.nn.rnn_cell.DropoutWrapper(lstm_cell, output_keep_prob= self.keep_prob) # rnn中保留的部分
    # 动态变化的RNN
    def add_dynamic_rnn(self):
        self.outputs, self.last_state = tf.nn.dynamic_rnn(
            ...
            dtype=tf.float32
        ) #cell 使用的是带Dropout的LSTMCell,输入源是经过embedding定处理过的X,长度是X的长度
    def add_output_layer(self):
        self.logits = tf.layers.dense(inputs= self.last_state.h,units= self.n_out,activation=tf.nn.relu,
        kernel_regularizer = tf.contrib.layers.l2_regularizer(0.003)) #全连接层,输入的是二维tensor,units 该层的神经单元结点数
    # 优化器，使用AdamOptimizer
    ...
    def fit(self, X, Y, val_data=None, n_epoch=10, batch_size=128, exp_decay=True,
        ...
        saver.save(self.sess,"/Users/Apple/datadata/wenben/rnn_test1/model_saved/model.ckpt")
        return log
...
with open("/Users/Apple/datadata/wenben/rnn_test1/regular_text/textlist_train.txt", 'rb') as f:
    textlist_train = pickle.load(f)
with open("/Users/Apple/datadata/wenben/rnn_test1/regular_text/textlist_test.txt", 'rb') as f:
    textlist_test = pickle.load(f)

print(len(train_texts),len(ind))
print(len(textlist_train))
...
result.to_csv("/Users/Apple/datadata/wenben/rnn_test1/model_saved/result.csv", sep=",")